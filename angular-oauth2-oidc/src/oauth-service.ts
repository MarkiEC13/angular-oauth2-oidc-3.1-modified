...
export class OAuthService
	...
	/*
		Timeout value is calculated wrong If user does a page reload.
	...
	*/
	...
	private calcTimeout(storedAt: number, expiration: number): number {
	  let tokenLifetime = expiration - storedAt;
	  let refreshTime = storedAt + (tokenLifetime * this.timeoutFactor);
	  let timeLeft = Math.max(0, refreshTime - +new Date);
	  return timeLeft;
	}
    ...
    private storeAccessTokenResponse(accessToken: string, refreshToken: string, expiresIn: number): void {
        ...
        this._storage.setItem('access_token_stored_at', '' + +new Date);
        if (expiresIn) {
    ...
    protected storeIdToken(idToken: ParsedIdToken) {
        ...
        this._storage.setItem('id_token_stored_at', '' + +new Date);
    }
